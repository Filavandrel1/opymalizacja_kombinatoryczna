...
// Zgodnosc z prawdziwym spektrum (bez bledow) jako porownanie multizbiorow k-merow.
// Zwraca: (liczba dopasowanych elementow, procent dopasowania).
static pair<int, double> policzZgodnoscZeSpektrumPrawdziwym(const vector<string>& spektrumPrawdziwe, const string& sekwencjaOdtworzona) {
    if (P.k <= 0 || sekwencjaOdtworzona.size() < static_cast<size_t>(P.k) || spektrumPrawdziwe.empty()) {
        return { 0, 0.0 };
    }

    unordered_map<string, int> licznikPrawdziwe;
    licznikPrawdziwe.reserve(spektrumPrawdziwe.size() * 2);
    for (const auto& oligo : spektrumPrawdziwe) {
        if (!oligo.empty()) {
            ++licznikPrawdziwe[oligo];
        }
    }

    unordered_map<string, int> licznikOdtworzone;
    const int k = P.k;
    const int n = static_cast<int>(sekwencjaOdtworzona.size());
    const int liczbaKmerow = max(0, n - k + 1);
    licznikOdtworzone.reserve(static_cast<size_t>(liczbaKmerow) * 2);
    for (int i = 0; i <= n - k; ++i) {
        string kmer = sekwencjaOdtworzona.substr(i, k);
        ++licznikOdtworzone[kmer];
    }

    int dopasowane = 0;
    for (const auto& kv : licznikPrawdziwe) {
        auto it = licznikOdtworzone.find(kv.first);
        if (it == licznikOdtworzone.end()) continue;
        dopasowane += min(kv.second, it->second);
    }
    ...
    double procent = 0.0;
    if (calkowita > 0) {
        procent = (static_cast<double>(dopasowane) / static_cast<double>(calkowita)) * 100.0;
    }
    return { dopasowane, procent };
}
...
static int uruchomBatchZPliku(const string& nazwaPliku, unsigned seed) {
    ...
    parametry.limitKandydatow = 2;
    const vector<int> liczbyMrowek = { 10, 20, 30, 50, 70, 100 };
    ...
    cout << "liczba_mrowek\tsrednie_pokrycie_z_bledami_procent\tzgodnosc_prawdziwe_spektrum_procent\tsredni_levenshtein" << endl;
    for (size_t idx = 0; idx < liczbyMrowek.size(); ++idx) {
        const int liczbaMrowek = liczbyMrowek[idx];
        parametry.liczbaMrowek = liczbaMrowek;
        ...
        for (int proba = 0; proba < liczbaProb; ++proba) {
            ...
            auto zgodnoscPrawdziwe = policzZgodnoscZeSpektrumPrawdziwym(I.spektrum, najlepszyGlobalny.sekwencja);
            sumaZgodnosciPrawdziwe += zgodnoscPrawdziwe.second;
        ...
        cout << liczbaMrowek << "\t" << sredniePokrycieZBledami << "\t" << sredniaZgodnoscPrawdziwe << "\t" << sredniLevenshtein << endl;
    }
